...
class Llm:
  ...
  def _check_network_assignments(self):
    ...
    # print("used networks ", used)
    # print("num_networks ", size)
    ...
    for tier_used, tier_size, tier in zip(
      ...
      if tier_used:
        # print('Tier Size', tier_size)
        # print('System Network size',  self.sys.get_network(tier).size) 
  ...
  def _compute_block_stats(self):
    ...
    self._block_fw_flops_energy = 0
    self._block_fw_mem_accessed = 0
    self._block_fw_mem_time = 0
    self._block_fw_mem_energy = 0
    ...
    self._block_re_flops_energy = 0
    self._block_re_mem_accessed = 0
    self._block_re_mem_time = 0
    self._block_re_mem_energy = 0
    ...
    self._block_agrad_flops_energy = 0
    self._block_agrad_mem_accessed = 0
    self._block_agrad_mem_time = 0
    self._block_agrad_mem_energy = 0
    ...
    for layer in self._llm_block:
      ...
      if self.exe.training:
        if layer.get_recompute_flag():
          ...
          self._block_re_flops_energy += self._block_re_flops_energy
  ...
  def _check_mem_caps(self):
    # print("Mem Tier1 Capacity", self.sys.mem1.capacity)
    if float(self.get_mem_tier1_cap_req()) > self.sys.mem1.capacity:
  ...
  # Getters for energy-related variables
  def get_block_fw_flops_energy(self):
      return torch.tensor(self._block_fw_flops_energy, dtype=torch.float32, requires_grad=True)

  def get_block_fw_mem_energy(self):
      return torch.tensor(self._block_fw_mem_energy, dtype=torch.float32, requires_grad=True)

  def get_block_re_flops_energy(self):
      return torch.tensor(self._block_re_flops_energy, dtype=torch.float32, requires_grad=True)

  def get_block_re_mem_energy(self):
      return torch.tensor(self._block_re_mem_energy, dtype=torch.float32, requires_grad=True)

  def get_block_agrad_flops_energy(self):
      return torch.tensor(self._block_agrad_flops_energy, dtype=torch.float32, requires_grad=True)

  def get_block_agrad_mem_energy(self):
      return torch.tensor(self._block_agrad_mem_energy, dtype=torch.float32, requires_grad=True)

  def get_block_wgrad_flops_energy(self):
      return torch.tensor(self._block_wgrad_flops_energy, dtype=torch.float32, requires_grad=True)

  def get_block_wgrad_mem_energy(self):
      return torch.tensor(self._block_wgrad_mem_energy, dtype=torch.float32, requires_grad=True)

  def get_block_optim_flops_energy(self):
      return torch.tensor(self._block_optim_flops_energy, dtype=torch.float32, requires_grad=True)

  def get_block_optim_mem_energy(self):
  ...
  def get_total_compute_energy(self):
    with torch.no_grad():
      total_energy = self.get_block_fw_flops_energy()
      total_energy += self.get_block_re_flops_energy() 
      total_energy += self.get_block_agrad_flops_energy() 
      total_energy += self.get_block_wgrad_flops_energy()        
      total_energy += self.get_block_optim_flops_energy() 
    return total_energy
  

  def get_total_mem_energy(self):
    with torch.no_grad():
      total_energy = self.get_block_fw_mem_energy()
      total_energy += self.get_block_agrad_mem_energy()
      total_energy += self.get_block_optim_mem_energy() 
      total_energy += self.get_block_wgrad_mem_energy() 
      total_energy += self.get_block_re_mem_energy()
    return total_energy
  ...
  def _get_mem_cap_reqs(self):
    ...
    # print('Tier 1 mem capacity requirement', tier1)
    # print('Tier 2 mem capacity requirement', tier2)
    return tier1, tier2
