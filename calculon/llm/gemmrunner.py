...
class GemmRunner(calculon.CommandLine):
  NAME = 'gemm'
  ALIASES = []

  @staticmethod
  def create_parser(subparser):
    sp = subparser.add_parser(GemmRunner.NAME, aliases=GemmRunner.ALIASES,
                              help='run a single llm calculation')
    sp.set_defaults(func=GemmRunner.run_command)
  ...
  def run_command(logger, args):
    ...
    app = Gemm.Application(app_json)
    exe = Gemm.Execution.from_json(exe_json)
    syst = System(sys_json)

    try:
      model = Gemm(app, logger)
      model.compile(syst, exe)
      model.run(syst)
    except Gemm.Error as error:
      print(f'ERROR: {error}')
      return -1
...
calculon.CommandLine.register(GemmRunner)
